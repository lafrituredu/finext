    ...
    <>
        ...
        <div className='p-6 sm:p-10 inter'>
            ...
            <div className='inter px-8 py-5 bg-sky-50 dark:bg-blue-950 dark:text-white rounded-2xl mb-5'>
                <p className='text-xl flex items-center gap-2'><InfoIcon />{t('alert_no_irpf.title')}</p>
                <p>{t('alert_no_irpf.message')}</p>
            </div>}
            ...
            <div id='toggle' className='relative bg-[#EFEFEF] dark:bg-[#0F1732] w-full px-2 py-1 rounded-3xl flex justify-between items-center gap-5 border border-[#0000001a] mb-4 montserrat'>
                <div id='1' onClick={(e) => setQuarter(Number(e.currentTarget.id))} className={`${quarter == 1 ? 'bg-[#FFF] dark:bg-[#1a2957] w-fit  rounded-2xl' : ''} flex gap-3 items-center justify-center text-center w-full px-2 py-1 transition-all ease-in-out duration-200 cursor-pointer`}>{t('quarters.T1')} {todayMonth < 3 && <span className='w-2 h-2 bg-primary rounded-2xl relative after:w-2 after:h-2 after:bg-primary after:rounded-2xl after:animate-ping after:top-0 after:left-0 after:absolute '/>}</div>
                <div id='2' onClick={(e) => setQuarter(Number(e.currentTarget.id))} className={`${quarter == 2 ? 'bg-[#FFF] dark:bg-[#1a2957] w-fit  rounded-2xl' : ''} flex gap-3 items-center justify-center text-center w-full px-2 py-1 transition-all ease-in-out duration-200 cursor-pointer`} >{t('quarters.T2')} {todayMonth >= 3 && todayMonth < 6 && <span className='w-2 h-2 bg-primary rounded-2xl relative after:w-2 after:h-2 after:bg-primary after:rounded-2xl after:animate-ping after:top-0 after:left-0 after:absolute '/>}</div>
                <div id='3' onClick={(e) => setQuarter(Number(e.currentTarget.id))} className={`${quarter == 3 ? 'bg-[#FFF] dark:bg-[#1a2957] w-fit  rounded-2xl' : ''} flex gap-3 items-center justify-center text-center w-full px-2 py-1 transition-all ease-in-out duration-200 cursor-pointer`} >{t('quarters.T3')} {todayMonth >= 6 && todayMonth < 9 && <span className='w-2 h-2 bg-primary rounded-2xl relative after:w-2 after:h-2 after:bg-primary after:rounded-2xl after:animate-ping after:top-0 after:left-0 after:absolute '/>}</div>
                <div id='4' onClick={(e) => setQuarter(Number(e.currentTarget.id))} className={`${quarter == 4 ? 'bg-[#FFF] dark:bg-[#1a2957] w-fit  rounded-2xl' : ''} flex gap-3 items-center justify-center text-center w-full px-2 py-1 transition-all ease-in-out duration-200 cursor-pointer`} >{t('quarters.T4')} {todayMonth >= 9 && <span className='w-2 h-2 bg-primary rounded-2xl relative after:w-2 after:h-2 after:bg-primary after:rounded-2xl after:animate-ping after:top-0 after:left-0 after:absolute '/>}</div>
                <div id='0' onClick={(e) => setQuarter(Number(e.currentTarget.id))} className={`${quarter == 0 ? 'bg-[#FFF] dark:bg-[#1a2957] w-fit  rounded-2xl' : ''} flex gap-3 items-center justify-center text-center w-full px-2 py-1 transition-all ease-in-out duration-200 cursor-pointer`} >{t('quarters.yearly')}</div>
            </div>
            ...
            <div className='inter *:inter w-full h-full border border-[#0000001a] rounded-2xl py-5 px-8 flex flex-col gap-5'>
                ...
                <div>
                    <p className='text-xl mb-2'>{t('IVA.title')}</p>
                    <div className='flex lg:flex-row flex-col gap-6 *:px-4 *:py-3 *:w-full *:flex *:justify-between *:rounded-2xl'>
                        <div className='dark:bg-green-200 dark:border-green-800 dark:text-black bg-green-50 border-[#E2E8F0] border'><span>{t('IVA.collectedVat')}</span> <span>{collectedVat}€</span></div>
                        <div className='dark:bg-red-200 dark:border-red-800 dark:text-black bg-red-50 border-[#E2E8F0] border'><span>{t('IVA.inputVat')}</span> <span>{inputVat}€</span></div>
                        <div className='dark:bg-blue-200 dark:border-blue-800 dark:text-black bg-blue-50 border-[#E2E8F0] border'><span>{t('IVA.toPayVat')}</span> <span>{collectedVat-inputVat}€</span></div>
                    </div>
                </div>
                <div className='lg:hidden block w-full h-px bg-gray-300'/>

                <div>
                    <p className='text-xl mb-2'>{t('IRPF+CUOTA.title')}</p>
                    <div className='flex lg:flex-row flex-col gap-6 *:px-4 *:py-3 *:w-full *:flex *:justify-between *:rounded-2xl'>
                        <div className='dark:bg-green-200 dark:border-green-800 dark:text-black bg-green-50 border-[#E2E8F0] border'><span>{t('IRPF+CUOTA.IRPF')}</span> <span>{payIRPF}€</span></div>
                        <div className='dark:bg-red-200 dark:border-green-800 dark:text-black bg-red-50 border-[#E2E8F0] border'><span>{t('IRPF+CUOTA.cuota')}</span> <span>{cuota}€</span></div>
                        <div className='dark:bg-blue-200 dark:border-green-800 dark:text-black bg-blue-50 border-[#E2E8F0] border'><span>{t('IRPF+CUOTA.irpf+cuota')}</span> <span>{(Number(irpf)/100)*( (incomes-collectedVat) - (expenses))+cuota}€</span></div>
                    </div>
                </div>
                <div className='lg:hidden block w-full h-px bg-gray-400'/>
                
                <div>
                    <p className='text-xl mb-2'>{t('SavingRecommendation')}</p>
                    <div className='flex lg:flex-row flex-col gap-6 *:px-4 *:py-3 *:w-full *:flex *:justify-between *:rounded-2xl'>
                        <div className='bg-orange-100 border border-orange-300 dark:bg-orange-200 dark:text-black'><span>{t('TotalToPay')} (IRPF {Number(irpf)}% + Cuota + IVA):</span> <span>{Math.abs(collectedVat-inputVat) + payIRPF! + cuota}€</span></div>
                    </div>
                </div>
            </div>
            ...
            {Number(irpf) != null && 
                <div className='inter px-8 py-5 bg-orange-50 dark:bg-orange-300 dark:text-black rounded-2xl mt-5'>
                    <p className='text-xl flex items-center gap-2'><InfoIcon />{t('alert_taxes.title')}</p>
                    <p>{t('alert_taxes.message')}</p>
                </div>
            }
        </div>
    </>
